...
def load_convert_h5(filepath):
    with h5py.File(filepath, "r") as f:
        ...
        proxy_ch_vars = [f"proxy_ch_{el}" for el in ["e", "pt", "eta", "sinphi", "cosphi"]]
        proxy_neut_vars = [f"proxy_neut_{el}" for el in ["e", "pt", "eta", "sinphi", "cosphi"]]

        pflow_data = s2u(f["regression"].fields(pflow_vars)[:])
        proxy_data = s2u(f["regression"].fields(proxy_vars)[:])
        proxy_ch_data = s2u(f["regression"].fields(proxy_ch_vars)[:])
        proxy_neut_data = s2u(f["regression"].fields(proxy_neut_vars)[:])

        # is_charged flag
        is_charged = None
        if "pflow_is_charged" in f["regression"].dtype.names:
            is_charged = s2u(f["regression"].fields(["pflow_is_charged"])[:]).squeeze(-1)
        ...
        proxy_ch_ptetaphi = np.stack(
            [
                proxy_ch_data[..., 1],
                proxy_ch_data[..., 2],
                np.arctan2(proxy_ch_data[..., 3], proxy_ch_data[..., 4]),
            ],
            axis=-1,
        )
        proxy_neut_ptetaphi = np.stack(
            [
                proxy_neut_data[..., 1],
                proxy_neut_data[..., 2],
                np.arctan2(proxy_neut_data[..., 3], proxy_neut_data[..., 4]),
            ],
            axis=-1,
        )
...
class PflowPredictionWriter(Callback):
    ...
    def on_test_batch_end(self, trainer, module, test_step_outputs, batch, batch_idx):
        ...
        for _, t in enumerate(["e", "pt", "eta", "sinphi", "cosphi"]):
            ...
            proxy_ch_data = None; proxy_neut_data = None
            if f"pflow_proxy_ch_{t}" in preds["regression"]:
                proxy_ch_data = preds["regression"][f"pflow_proxy_ch_{t}"].cpu().float().unsqueeze(-1)
                proxy_neut_data = preds["regression"][f"pflow_proxy_neut_{t}"].cpu().float().unsqueeze(-1)
            if t in self.var_transform:
                ...
                if proxy_ch_data is not None and proxy_neut_data is not None:
                    proxy_ch_data = self.var_transform[t].inverse_transform(proxy_ch_data)
                    proxy_neut_data = self.var_transform[t].inverse_transform(proxy_neut_data)
            ...
            if proxy_ch_data is not None and proxy_neut_data is not None:
                to_write["regression"][f"proxy_ch_{t}"] = u2s(
                    proxy_ch_data.numpy(),
                    dtype=np.dtype([(f"proxy_ch_{t}", np.float32)]),
                )
                to_write["regression"][f"proxy_neut_{t}"] = u2s(
                    proxy_neut_data.numpy(),
                    dtype=np.dtype([(f"proxy_neut_{t}", np.float32)]),
                )

        if f"pflow_is_charged" in preds["regression"]:
            is_charged_data = preds["regression"][f"pflow_is_charged"].cpu().float().unsqueeze(-1)
            to_write["regression"][f"pflow_is_charged"] = u2s(
                is_charged_data.numpy(),
                dtype=np.dtype([(f"pflow_is_charged", np.float32)]),
            )
        ...
        with uproot.recreate(root_path) as f:
            f["event_tree"] = {
                ...
                "proxy": {
                    ...
                    "is_charged": ak.Array(is_charged)
                },
                "proxy_ch": {
                    "pt": ak.Array(proxy_ch_ptetaphi[..., 0]),
                    "eta": ak.Array(proxy_ch_ptetaphi[..., 1]),
                    "phi": ak.Array(proxy_ch_ptetaphi[..., 2]),
                },
                "proxy_neut": {
                    "pt": ak.Array(proxy_neut_ptetaphi[..., 0]),
                    "eta": ak.Array(proxy_neut_ptetaphi[..., 1]),
                    "phi": ak.Array(proxy_neut_ptetaphi[..., 2]),
                },
